...
class ReservationsSubScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      stream: Firestore.instance.collection('events').document(eventID).collection('guests').orderBy('gName').snapshots(),
      builder: (context, snapshot) {
        if(snapshot.connectionState == ConnectionState.waiting) return Center(child : CircularProgressIndicator());
        return snapshot.data.documents.length == 0 ? Center(child : Text('There are no reservations for this event.')) :
        ListView.builder(
                shrinkWrap: true,
                ...
                itemBuilder: (ctx, index) => GuestTile(
                  ...
                  gMobileNumber: snapshot.data.documents[index]['gMobileNumber'],
                  ...
                  isCheckedIn: snapshot.data.documents[index]['isCheckedIn'],
                  isAdmin: isAdmin,
